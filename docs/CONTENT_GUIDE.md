...
- **Estação:** adicione ID ao tipo `StationId`, registre o frame em `src/assets/pixel/manifest.ts` e defina posição, tamanho, frente e pontos de interação em `src/content/stations/stations.ts`. Cada ponto deve ser caminhável.
...
- **Móvel/mesa:** siga `createTables()` em `game/map/initialMap.ts`, com footprint, células ocupadas, cadeiras, aproximações, âncora e capacidade. A base desenhada deve coincidir com o polígono do footprint. Execute os testes de mapa e confira o modo técnico.
