...
import { INITIAL_TUTORIAL_STEPS, JOURNEY_CHAPTER_LEVELS, acknowledgeTutorialStep, reconcileTutorial } from '../game/tutorial/Tutorial008Service';
...
describe('0.0.8 econômica sem ingredientes', () => {
  ...
  it('tutorial possui 16 etapas, etapa 9 econômica e contratação de limpeza', () => { expect(INITIAL_TUTORIAL_STEPS).toHaveLength(16); expect(INITIAL_TUTORIAL_STEPS[8].id).toBe('cost-and-time'); expect(INITIAL_TUTORIAL_STEPS[8].objective).not.toMatch(/ingrediente/i); expect(INITIAL_TUTORIAL_STEPS.some((step) => step.id === 'hire-cleaner')).toBe(true); });
