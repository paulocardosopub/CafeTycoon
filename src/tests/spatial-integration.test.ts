...
describe('correção estrutural — 10 fluxos de integração', () => {
  ...
  it('FLUXO I — mobile salva toque, arraste e rotação sem check e X', () => { const ui=readFileSync('src/ui/ConstructionShop.ts','utf8'); const scene=readFileSync('src/scenes/RestaurantScene.ts','utf8'); expect(ui).not.toContain('data-editor-action="confirm-item"'); expect(ui).not.toContain('data-editor-action="cancel-item"'); expect(ui).toContain('Posição salva automaticamente.'); expect(ui).toContain('previewFurnitureRotation'); expect(scene).toContain('construction:world-drag'); });
