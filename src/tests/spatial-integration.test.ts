...
describe('correção estrutural — 10 fluxos de integração', () => {
  ...
  it('FLUXO F — fogão gira 2×1 para 1×2 com WorkSlots', () => { const state=createDefaultState(0); const editor=new ConstructionEditor(state); expect(editor.previewFurnitureRotation('furniture:a1').ok).toBe(true); expect(editor.draft.construction.placedFurniture.find((item)=>item.id==='furniture:a1')?.footprint).toEqual({width:1,depth:2}); expect(editor.confirmFurnitureEdit().ok).toBe(true); });
